...
import { InterpolationComponent, PhysicsBodyComponent, TransformComponent } from '@phalanx-engine/physics';
import {
  TeamComponent,
  UnitTypeComponent,
  StatsComponent,
...
export function assembleUnit(
  ...
  const fpRotation = teamId === 0 ? FPQuaternion.Identity() : FPQuaternion.FromYaw(FP.Pi);
  ...
  entity.addComponent(new UnitTypeComponent(def.type, FP.FromFloat(def.detectionRange)));
  ...
  if (refs.spawnPoint) entity.addComponent(new SpawnPointComponent(refs.spawnPoint.marker));
  if (def.hasAutoAttackTimer) entity.addComponent(new AutoAttackTimerComponent());
  if (def.hasCubeState) entity.addComponent(new CubeStateComponent());
  if (def.aura) entity.addComponent(new HealAuraComponent(def.aura, refs.auraRing ?? null));
}
