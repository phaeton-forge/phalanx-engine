...
import type { IAfterFrame, SoAComponentStore, SystemContext } from 'phalanx-ecs';
...
export class RotationSystem extends GameSystem implements IAfterFrame {
  ...
  public afterFrame(_alpha: number, _dt: number): void {
    const visualRotationY = this.transformStore.arrays.visualRotationY;
    const units = this.entityManager.queryEntities(
      ComponentType.Mesh,
      ComponentType.Transform,
    ...
    for (const unit of units) {
      ...
      mesh.root.rotation.y = visualRotationY[unitIndex];
    }
  }
